...
from dataset_builder import BrainDataset_BraTS2020, BrainDatasetVal_BraTS2020, RandomCrop_BraTS2020, ToTensor_BraTS2020,ValToTensor_BraTS2020
...
# prepare dataset
# build train dataset
train_dataset = BrainDataset_BraTS2020(root_dir=train_data_dir,
                             transform=transforms.Compose([
                                 RandomCrop_BraTS2020(patch_size),
                                 ToTensor_BraTS2020()
                             ]))

val_dataset = BrainDatasetVal_BraTS2020(root_dir=val_data_dir,
                              test_instance_id=test_instance_id,
                              patch_size=args.patch_size,
                              overlap_step=args.overlap_step,
                              transform=transforms.Compose([
                                  ValToTensor_BraTS2020()
                              ]))
    ...
    for epoch in range(begin_epoch, num_epochs):
        ...
        for itr, sample in enumerate(dataloaders[phase]):
            ...
            with torch.autograd.set_grad_enabled(phase == 'train'):
                ...
                count_map_out = getCounting(label_pred)
        ...
        with torch.autograd.set_grad_enabled(False):
            for sample in dataloaders[phase]:
                ...
                batch_corrects += torch.mean(pred_arr.float()).item() * sample["image_t1c"].size(0)
