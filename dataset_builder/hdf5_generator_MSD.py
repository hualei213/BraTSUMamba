...
def write_training_examples(raw,label, original_shape, cut_size, output_file):
    ...
    dset_image_t1c = f.create_dataset(name="image",
                                  data=raw)
...
def write_validation_examples(raw,
                        original_shape,
                        patch_size,
                        overlap_stepsize,
                        output_file):
    ...
    patch_ids = prepare_validation(raw, patch_size, overlap_stepsize)
    ...
    dset_patch_ids_t1c = f.create_dataset(name="patch_id",
                                      data=patch_ids)
    # dset_patch_ids_t1n = f.create_dataset(name="patch_id_t1n",
    #                                   data=patch_ids_t1n)
    # dset_patch_ids_t2f = f.create_dataset(name="patch_id_t2f",
    #                                   data=patch_ids_t2f)
    # dset_patch_ids_t2w = f.create_dataset(name="patch_id_t2w",
    #                                   data=patch_ids_t2w)
    ...
    dset_original_shape = f.create_dataset(name="original_shape",
                                           data=original_shape)
    # chuncked storage
    dset_image = f.create_dataset(name="image",
                                  shape=(patch_size,
                                         patch_size,
                                         patch_size * len(patch_ids),
                                         4),
                                  chunks=(patch_size,
                                          patch_size,
                                          patch_size,
    ...
    for i in range(len(patch_ids)):
        (d, h, w) = patch_ids[i]
        _raw = raw[d:d + patch_size, h:h + patch_size, w:w + patch_size, :]
        dset_image[0:patch_size, 0:patch_size, i*patch_size:(i+1)*patch_size, :] = _raw
