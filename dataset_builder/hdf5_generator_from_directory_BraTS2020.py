...
from hdf5_generator_BraTS2020 import write_training_examples, \
...
def load_subject(case_path_t1c,case_path_t1n,case_path_t2f,case_path_t2w, brain_mask_path):
    """
    ...
    inputs_label[inputs_label == 4.0] = 3.0        #BraTS2020  label需要把4.0改为3.0
    ...
    return inputs_T1c,inputs_T1n,inputs_T2f,inputs_T2w, inputs_label
...
def generate_files(raw_data_dir, output_path,
    ...
    for case_id in all_id_list:
        ...
        if file_not_exist_flag:
            ...
            case_name_t2w = case_id + '_t2.nii.gz'     #BraTS2020
            # brain mask name
            brain_mask_name = case_id + '_seg.nii.gz'   #BraTS2020
        ...
        if case_id in valid_id_list:
            ...
            write_validation_examples(T1c=_T1c, T1n=_T1n, T2f=_T2f, T2w=_T2w,
                                      original_shape=original_shape,
                                      patch_size=patch_size,
                                      overlap_stepsize=overlap_stepsize,
        ...
        if case_id in test_id_list:
            print('Create the test file:')
            write_test_examples(T1c=_T1c, T1n=_T1n, T2f=_T2f, T2w=_T2w,
                                original_shape=original_shape,
                                patch_size=patch_size,
                                overlap_stepsize=overlap_stepsize,
                                output_file=test_filename)
