...
class MambaEncoder(nn.Module):
    ...
    def forward_features(self, x):
        ...
        for i in range(4):
            ...
            #频域分支
            x_fd = self.stages_FD[i](x)
            x_fd = self.stages[i](x_fd)
            #空间域分支
            x_sd = self.gscs[i](x)
            x_sd = self.stages[i](x_sd)
            #融合
            x=x_fd+x_sd
...
class UNeXt_3D_mlp_attention_Parallelfdgmsm_fsca_pc(nn.Module):
    def __init__(self, n_channels, n_classes):
        super(UNeXt_3D_mlp_attention_Parallelfdgmsm_fsca_pc, self).__init__()
