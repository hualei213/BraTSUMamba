...
class UNeXt3D_mlp_attention(nn.Module):
    def __init__(self, n_channels, n_classes):
        ...
        self.conv5_1 = inconv_5(n_channels, 16)
        self.conv5_2 = inconv_5(16, 32)
        self.conv5_3 = inconv_5(32, 64)

        self.conv7_1 = inconv_7(n_channels, 16)
        self.conv7_2 = inconv_7(16, 32)
        self.conv7_3 = inconv_7(32, 64)

        self.conv_adjust_1 = nn.Conv3d(in_channels=n_channels, out_channels=16, kernel_size=1)
        self.conv_adjust_2 = nn.Conv3d(in_channels=16, out_channels=32, kernel_size=1)
        self.conv_adjust_3 = nn.Conv3d(in_channels=32, out_channels=64, kernel_size=1)
    ...
    def forward_encoder(self,x):
        ...
        cov3_out = self.dow_conv1(x)
        cov5_out = self.conv5_1(x)
        cov7_out = self.conv7_1(x)
        out = cov3_out+cov5_out+cov7_out
        out_norm=self.Bn_dow_conv1(out)
        out = F.relu6(out_norm)
        out=F.max_pool3d(out, 2, 2)
        x_c = self.conv_adjust_1(x)
        x_p = F.max_pool3d(x_c,2,2)
        out = out+x_p
        # out = self.mlp_att1(out, mlp1)
        t1 = out                     #encoder_1

        out_m1 = mamba_outs[0]
        cov3_out = self.dow_conv2(out_m1)
        cov5_out = self.conv5_2(out_m1)
        cov7_out = self.conv7_2(out_m1)
        out = cov3_out + cov5_out + cov7_out
        out_norm = self.Bn_dow_conv2(out)
        out = F.relu6(out_norm)
        out = F.max_pool3d(out, 2, 2)
        x_c = self.conv_adjust_2(out_m1)
        x_p = F.max_pool3d(x_c, 2, 2)
        out = out + x_p
        ...
        out_m2 = mamba_outs[1]
        cov3_out = self.dow_conv3(out_m2)
        cov5_out = self.conv5_3(out_m2)
        cov7_out = self.conv7_3(out_m2)
        out = cov3_out + cov5_out + cov7_out
        out_norm = self.Bn_dow_conv3(out)
        out = F.relu6(out_norm)
        out = F.max_pool3d(out, 2, 2)
        x_c = self.conv_adjust_3(out_m2)
        x_p = F.max_pool3d(x_c, 2, 2)
        out = out + x_p
