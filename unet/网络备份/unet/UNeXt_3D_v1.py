...
class UNeXt3D(nn.Module):
    ...
    def forward(self, x):
        ...
        ### Tokenized MLP Stage
        ### Token Mlp domain 1
        out, H, W, D = self.patch_embed1(out) # out: ? N 128 N=H/16 w/16 D/16
        out = self.block1(out, H, W, D)  # out: ? N 128
        out = self.norm1(out)# out: ? N 128
        out = out.reshape(B, H, W, D, -1).permute(0, 4, 1, 2, 3).contiguous()
        t4 = out# ? 128 H/16 W/16 D/16

        # Token Mlp domain 2
        out, H, W, D = self.patch_embed2(out)  # out:? N 256
        out = self.block2(out, H, W, D) # out:? N 256
        out = self.norm2(out)
        out = out.reshape(B, H, W, D, -1).permute(0, 4, 1, 2, 3).contiguous() # ? 256 H/32 W/32 D/32

        ### up stage
        # up1
        out = F.relu6(F.interpolate(self.dbn1(self.decoder1(out)), scale_factor=(2, 2, 2), mode='trilinear'))# ? 128 H/16 W/16 D/16
        out = torch.add(out, t4)
        _, _, H, W, D = out.shape
        out = out.flatten(2).transpose(1, 2)  # ? N 128
        out = self.dblock1(out, H, W, D)# ? N 128
        out = self.dnorm1(out)
        out = out.reshape(B, H, W, D, -1).permute(0, 4, 1, 2, 3).contiguous()# ? 128 H/16 W/16 D/16
        ...
        out = self.dblock2(out, H, W, D)#? N 64
        out = self.dnorm2(out)
        out = out.reshape(B, H, W, D, -1).permute(0, 4, 1, 2, 3).contiguous()#? 64 H/8 W/8 D/8
