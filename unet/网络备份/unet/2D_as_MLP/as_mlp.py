...
class DoubleDeASBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, shift_size,
        ...
        self.squeeze1=nn.Conv2d(self.dim,self.dim,1,1,0)
        self.squeeze2=nn.Conv2d(self.dim,self.dim//2,1,1,0)
        # patch merging layer
...
class AS_MLP(nn.Module):
    def __init__(self, img_size=128, patch_size=4, in_chans=1, num_classes=2,
                 embed_dim=64, depths=[2, 2, 2, 2],
        ...
        for i_layer in range(self.num_layers-1):
            dlayer = DoubleDeASBlock(dim=int(embed_dim * 2 ** (self.num_layers-1-i_layer)),
                               input_resolution=(patches_resolution[0] // (2 ** i_layer),
                                                 patches_resolution[1] // (2 ** i_layer)
                                                 ),
        ...
        self.up_scale = nn.Upsample(scale_factor=patch_size, mode='bilinear', align_corners=True)
        self.head = nn.Sequential(
            nn.Conv2d(self.embed_dim, self.embed_dim // 2, 1, 1, 0),
            nn.Conv2d(self.embed_dim // 2, num_classes, 1, 1, 0),
            nn.Sigmoid()
