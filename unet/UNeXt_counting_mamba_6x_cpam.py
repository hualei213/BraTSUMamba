...
class channel_attention(nn.Module):
    ...
    def forward(self, x):
        ...
        y = y.view(y.size(0), y.size(1), -1).transpose(1, 2)  # 调整形状为 (B, 1, C)
        y = self.conv(y).transpose(1, 2).view(x.size(0), x.size(1), 1, 1, 1)  # 1D卷积后恢复形状
...
class local_attention(nn.Module):
    def __init__(self, channel, reduction=16):
        ...
        # 通道降维的 1x1x1 卷积
        self.conv_1x1 = nn.Conv3d(in_channels=channel,
        ...
        # 深度注意力生成
        self.F_d = nn.Conv3d(channel // reduction, channel, kernel_size=1, bias=False)
    ...
    def forward(self, x):
        ...
        # 高度方向特征提取
        x_h = torch.mean(x, dim=3, keepdim=True).mean(dim=4, keepdim=True).permute(0, 1, 3, 4, 2)  # (B, C, 1, 1, H)
        # 宽度方向特征提取
        x_w = torch.mean(x, dim=2, keepdim=True).mean(dim=4, keepdim=True).permute(0, 1, 2, 4, 3)  # (B, C, 1, 1, W)

        # 深度方向特征提取
        x_d = torch.mean(x, dim=2, keepdim=True).mean(dim=3, keepdim=True)  # (B, C, 1, 1, D)

        # 拼接高、宽、深方向信息
        x_cat = torch.cat((x_h, x_w, x_d), dim=4)  # (B, C, 1, 1, H+W+D)


        x_cat = self.relu(self.bn(self.conv_1x1(x_cat)))

        # 分离
        x_cat_h, x_cat_w, x_cat_d = x_cat.split([H, W, D], dim=4)

        # 注意力生成
        s_h = self.sigmoid_h(self.F_h(x_cat_h))  # (B, C, H, 1, 1)
        s_w = self.sigmoid_w(self.F_w(x_cat_w))  # (B, C, 1, W, 1)
        s_d = self.sigmoid_d(self.F_d(x_cat_d))  # (B, C, 1, 1, D)

        # 应用注意力
        out = x * s_h.expand_as(x) * s_w.expand_as(x) * s_d.expand_as(x)
        return out
...
class CPAM(nn.Module):
    def __init__(self, ch):
        super().__init__()
        self.channel_attention = channel_attention(ch)
        self.local_attention = local_attention(ch)
...
class UNeXt_counting_mamba_6x_cpam(nn.Module):
    def __init__(self, n_channels, n_classes):
        super(UNeXt_counting_mamba_6x_cpam, self).__init__()
...
if __name__ == "__main__":
    ...
    model = UNeXt_counting_mamba_6x(n_channels=4, n_classes=4)
    model = model.cuda()
