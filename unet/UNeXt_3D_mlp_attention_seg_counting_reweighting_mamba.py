...
from unet.fusiontrans import Bottleneck, Weight_Attention

from timm.models.layers import DropPath
from unet.Mamba_my import Mamba
...
class forward_encoder(nn.Module):
    def __init__(self):
        super(forward_encoder,self).__init__()
        ...
        ###
        self.mlp1 = MLP(num_i=1, num_h=32, num_o=16)
    ...
    def forward(self,x):
        ...
        x_encoder = (t1,t2,t3,t4,out)
        return x_encoder


class UNeXt3D_mlp_attention_reweight(nn.Module):
    def __init__(self, n_channels, n_classes):
        super(UNeXt3D_mlp_attention_reweight, self).__init__()

        self.basic_dims = 16
        self.transformer_basic_dims = 512
        self.mlp_dim = 4096
        self.num_heads = 8
        self.depth = 3
        self.num_modals = 4
        ...
        self.t1c_encoder = forward_encoder()
        self.t1n_encoder = forward_encoder()
        self.t2w_encoder = forward_encoder()
        self.t2f_encoder = forward_encoder()

        self.fusion_Conv3D_1 = nn.Conv3d(in_channels=
                                         64, out_channels=16, kernel_size=3, padding=1, stride=1)
        self.fusion_Conv3D_2 = nn.Conv3d(in_channels=
                                         128, out_channels=32, kernel_size=3, padding=1, stride=1)
        self.fusion_Conv3D_3 = nn.Conv3d(in_channels=
                                         256, out_channels=64, kernel_size=3, padding=1, stride=1)
        self.fusion_Conv3D_4 = nn.Conv3d(in_channels=
                                         512, out_channels=128, kernel_size=3, padding=1, stride=1)
        self.fusion_Conv3D_5 = nn.Conv3d(in_channels=
                                         1024, out_channels=256, kernel_size=3, padding=1, stride=1)




        self.pos = nn.Parameter(torch.zeros(1, (self.patch_size ** 3) * 5, self.basic_dims * 16))
        self.fusion = nn.Parameter(
            nn.init.normal_(torch.zeros(1, self.patch_size ** 3, self.basic_dims * 16), mean=0.0, std=1.0))  # (1,125,256)
    ...
    def forward_fusion(self,x):
        ...
        basic_dims = self.basic_dims


        t1c_x1, t1c_x2, t1c_x3, t1c_x4, t1c_x5 = self.t1c_encoder(x[:, 0:1, :, :, :])
        t1n_x1, t1n_x2, t1n_x3, t1n_x4, t1n_x5 = self.t1n_encoder(x[:, 1:2, :, :, :])
        t2w_x1, t2w_x2, t2w_x3, t2w_x4, t2w_x5 = self.t2w_encoder(x[:, 2:3, :, :, :])
        t2f_x1, t2f_x2, t2f_x3, t2f_x4, t2f_x5 = self.t2f_encoder(x[:, 3:4, :, :, :])




        x_bottle = (t1c_x5, t1n_x5, t2w_x5, t2f_x5)

        B = x.size(0)
        fusion = torch.tile(self.fusion, [B, 1, 1])

        t1c_trans, t1n_trans, t2w_trans, t2f_trans, fusion_trans, attn = self.Bottleneck(x_bottle, fusion, self.pos)

        t1c_tra = t1c_trans.view(x.size(0), patch_size, patch_size, patch_size, basic_dims * 16).permute(0, 4, 1, 2, 3).contiguous()
        t1n_tra = t1n_trans.view(x.size(0), patch_size, patch_size, patch_size, basic_dims * 16).permute(0, 4, 1, 2, 3).contiguous()
        t2w_tra = t2w_trans.view(x.size(0), patch_size, patch_size, patch_size, basic_dims * 16).permute(0, 4, 1, 2, 3).contiguous()
        t2f_tra = t2f_trans.view(x.size(0), patch_size, patch_size, patch_size, basic_dims * 16).permute(0, 4, 1, 2, 3).contiguous()
        fusion_tra = fusion_trans.view(x.size(0), patch_size, patch_size, patch_size, basic_dims * 16).permute(0, 4, 1, 2,3).contiguous()

        de_x5 = (t1c_tra, t1n_tra, t2w_tra, t2f_tra)
        de_x4 = (t1c_x4, t1n_x4, t2w_x4, t2f_x4)
        de_x3 = (t1c_x3, t1n_x3, t2w_x3, t2f_x3)
        de_x2 = (t1c_x2, t1n_x2, t2w_x2, t2f_x2)
        de_x1 = (t1c_x1, t1n_x1, t2w_x1, t2f_x1)

        de_x1, de_x2, de_x3, de_x4, de_x5 = self.weight_attention(de_x1, de_x2, de_x3, de_x4, de_x5, attn)

        de_x1 = torch.cat(de_x1,dim=1)
        de_x1 = self.fusion_Conv3D_1(de_x1)
        de_x2 = torch.cat(de_x2,dim=1)
        de_x2 = self.fusion_Conv3D_2(de_x2)
        de_x3 = torch.cat(de_x3, dim=1)
        de_x3 = self.fusion_Conv3D_3(de_x3)
        de_x4 = torch.cat(de_x4, dim=1)
        de_x4 = self.fusion_Conv3D_4(de_x4)
        de_x5 = torch.cat(de_x5, dim=1)
        de_x5 = self.fusion_Conv3D_5(de_x5)

        x_reweighted = (de_x1,de_x2,de_x3,de_x4,de_x5)
        return x_reweighted
    ...
    def forward(self, source):
        # x_encoder=self.forward_encoder(source)
        x_reweightted = self.forward_fusion(source)
        out = self.forward_decoder(x_reweightted)
        x = out
        return x
...
if __name__ == "__main__":
    ...
    model = UNeXt3D_mlp_attention(n_channels=1, n_classes=4)
    model = model.cuda()
