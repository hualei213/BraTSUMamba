...
class FeatureFusion3D(nn.Module):
    def __init__(self, channels,stage):
        ...
        self.Fre_w = Spectral_Layer(dim=channels,stage=stage)
    ...
    def forward(self, x):
        ...
        #Frequency feature branch
        frequency_features = self.Fre_w(x)

        # Element-wise addition
        combined_features = global_features + spatial_features + frequency_features  # Broadcasting for (B, C, H, W, D)
...
class Spectral_Layer(nn.Module):
    def __init__(self, dim, stage=1, in_shape=[256,256,256]):
        super().__init__()
        self.dim = dim
...
class MambaLayer(nn.Module):
    def __init__(self, dim, d_state=16, d_conv=4, expand=2, num_slices_small=None, stage=1):
        super().__init__()
        self.dim = dim
        ...
        self.fusion = FeatureFusion3D(channels=dim,stage=stage)
...
class MambaEncoder(nn.Module):
    def __init__(self, in_chans=4, depths=[2, 2, 2, 2], dims=[16, 32, 64, 128, 256],drop_path_rate=0., layer_scale_init_value=1e-6, out_indices=[0, 1, 2, 3]):
        ...
        for i in range(4):
            ...
            stage = nn.Sequential(
                *[MambaLayer(dim=dims[i],stage=i, num_slices_small=num_slices_small_list[i]) for j in range(depths[i])]
            )
...
class gmsm_4x_scaf_pc_fdfusion(nn.Module):
    def __init__(self, n_channels, n_classes):
        super(gmsm_4x_scaf_pc_fdfusion, self).__init__()
...
if __name__ == "__main__":
    ...
    img = torch.ones([1, 4, 256, 256, 256]).cuda()
