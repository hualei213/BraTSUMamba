...
# 拉普拉斯金字塔分解（3D 版）
def laplacian_pyramid_3d(image, levels=1):
    current = image
    pyramid = []
...
class GSC(nn.Module):
    def __init__(self, in_channles) -> None:
        ...
        self.proj5 = nn.Conv3d(in_channles, in_channles, kernel_size=5, padding=2)
        self.norm5 = nn.InstanceNorm3d(in_channles)
        self.nonliner5 = nn.ReLU()

        self.proj7 = nn.Conv3d(in_channles, in_channles, kernel_size=7, padding=3)
    ...
    def forward(self, x):
        ...
        x5 = self.proj5(x)
        x5 = self.norm5(x5)
        x5 = self.nonliner5(x5)

        x7 = self.proj7(x)
        x7 = self.norm7(x7)
        x7 = self.nonliner7(x7)
...
class gmsm_4x_pc_l(nn.Module):
    def __init__(self, n_channels, n_classes):
        super(gmsm_4x_pc_l, self).__init__()
        ...
        self.conv_fre111_0 = nn.Conv3d(4, 4, kernel_size=1)

        self.conv_fre_final = nn.Conv3d(4, 4, kernel_size=1)
    ...
    def forward_decoder(self,out,t1,t2,t3,t4):
        ...
        out = self.conv_4(out)
        # low_f4 = laplacian_pyramid_3d(out,1)
        # low_f4 = self.conv_fre111_4(low_f4)
        low_f4 = out
        out = out + self.MDAF4(out,low_f4)
        #######################################################
        # up 2
        out = F.relu6(
        ...
        # low_f3 = laplacian_pyramid_3d(out, 1)
        # low_f3 = self.conv_fre111_3(low_f3)
        low_f3 = out
        out = out + self.MDAF3(out, low_f3)
        ...
        # low_f2 = laplacian_pyramid_3d(out, 1)
        # low_f2 = self.conv_fre111_2(low_f2)
        low_f2 = out
        out = out + self.MDAF2(out, low_f2)
        ...
        # low_f1 = laplacian_pyramid_3d(out, 1)
        # low_f1 = self.conv_fre111_1(low_f1)
        # low_f1 = out
        # out = out + self.MDAF1(out, low_f1)
        ...
        # low_f0 = laplacian_pyramid_3d(out, 1)
        # low_f0 = self.conv_fre111_0(low_f0)
        # low_f0 = out
        # out = out + self.MDAF0(out, low_f0)
        # out = self.conv_fre_final(out)
...
if __name__ == "__main__":
    img = torch.ones([4, 4, 128, 128, 128]).cuda()
