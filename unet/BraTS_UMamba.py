...
class Bi_granularity_Mamba(nn.Module):
    def __init__(self, in_chans=4, depths=[2, 2, 2, 2], dims=[16, 32, 64, 128],drop_path_rate=0., layer_scale_init_value=1e-6, out_indices=[0, 1, 2, 3]):
        ...
        for i in range(4):
            ...
            stage = nn.Sequential(
                *[AFF_MambaLayer(dim=dims[i], num_slices_small=num_slices_small_list[i]) for j in range(depths[i])]
            )
...
class BraTSUMamba(nn.Module):
    def __init__(self, n_channels, n_classes):
        ...
        self.vim = Bi_granularity_Mamba(in_chans,
                                depths=depths,
                                dims=feat_size,
