...
def getCounting(labels,mask):
    B = labels.shape[0]
    count_list = []
    for i in range(B):
        clabel = labels[i].clone()
        clabel = (clabel > 0).float()  # 二值
        clabel = clabel.reshape(1, 1, labels.shape[2] // 16, 16, labels.shape[3] // 16, 16, labels.shape[4] // 16, 16).sum(dim=(3, 5, 7))
        clabel = clabel / (16 ** 3)
        if mask:
            clabel = (clabel > 0).float()
        else:
            average = clabel.sum() / (8 ** 3)
            clabel = (clabel > (0.01 * average)).float()
        count_list.append(clabel)
    count_map = torch.cat(count_list,dim=0)
    return count_map


# def getCounting(labels):
#     B = labels.shape[0]
#     count_list = []
#     for i in range(B):
#         clabel = labels[i].clone()
#         clabel = (clabel > 0).float()  # 二值
#         clabel = clabel.reshape(1, 1, labels.shape[2] // 4, 4, labels.shape[3] // 4, 4, labels.shape[4] // 4, 4).sum(dim=(3, 5, 7))
#         clabel = clabel / (4 ** 3)
#         average = clabel.sum() / (32 ** 3)
#         s = (clabel > 0).float()
#         clabel = (clabel > (0.1 * average)).float()
#         count_list.append(clabel)
#     count_map = torch.cat(count_list,dim=0)
#     return count_map
...
def val_sample(sample, model, device):
    ...
    # input_images = sample['image'].to(device)
    # input_mask = sample['label'].to(device)
    # get ids of input patches
    # patch_id_t1c = sample['patch_id_t1c']
    #
    # patch_id_t1c = patch_id_t1c.type(torch.int16)
    # patch_id_t1c = torch.squeeze(patch_id_t1c, dim=0)
    ...
    # batch_group_mile_stone = list(range(0, patch_id_t1c.size(0), batch_size))
    # if patch_id_t1c.size(0) % batch_size != 0:
    #     # add id of the final patch
    #     batch_group_mile_stone.append(patch_id_t1c.size(0))
    ...
    for i in range(len(batch_group_mile_stone)):
        ...
        if i == len(batch_group_mile_stone) - 1:
            ...
            # input_patches_id = [patch_id_t1c[j, :].numpy()
            #                     for j in list(range(batch_group_mile_stone[i],
            #                                         batch_group_mile_stone[i + 1]))]
            ...
            # input_patches = torch.cat([input_patches_t1c,input_patches_t1n,input_patches_t2f,input_patches_t2w],dim=1)
            # predict
...
def train_model(model,
    ...
    for epoch in range(begin_epoch, num_epochs):
        ...
        for itr, sample in enumerate(dataloaders[phase]):
            ...
            # image_t2f = sample['image_t2f'].to(device)
            # image_t2w = sample['image_t2w'].to(device)
            # input_images = torch.cat([image_t1c, image_t1n, image_t2f, image_t2w], dim=1)
            input_images = sample['image'].to(device)
            ...
            # bmap_orig = dmap.clone().reshape(labels.shape[0],1,labels.shape[2]//16,16,labels.shape[3]//16,16,labels[4]//16,16).sum(dim=(2, 4))
            # clabel = labels.clone()
            # clabel = (clabel > 0).float()  #二值
            # clabel = clabel.reshape(clabel.shape[0],1,labels.shape[2]//16,16,labels.shape[3]//16,16,labels.shape[4]//16,16).sum(dim=(3,5,7))
            # clabel = clabel / (16 ** 3)
            # average = clabel.sum() / (8 ** 3)
            # s = (clabel>0).float()
            # clabel = (clabel > (0.1 * average)).float()

            clabel_map = getCounting(labels,mask=True)
            ...
            with torch.autograd.set_grad_enabled(phase == 'train'):
                ...
                count_map_out = getCounting(label_pred,mask=False)
                loss_cls = F.binary_cross_entropy(clabel_map, count_map_out)
                ...
                acc_wt = bin_acc(infer_wt, label_wt)
                acc_et = bin_acc(infer_et, label_et)
                acc_tc = bin_acc(infer_tc, label_tc)
                acc = (acc_et+acc_wt+acc_tc)/3.0
                ...
                if phase == 'train':
                    ...
                    loss = loss_seg + lambda_weight * loss_cls
                    loss.backward()
                    # update model
                    optimizer.step()
        ...
        with torch.autograd.set_grad_enabled(False):
            for sample in dataloaders[phase]:
                ...
                # statistics
                # batch_loss += loss.item() * sample["image_t1c"].size(0)
